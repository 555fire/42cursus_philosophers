...
static t_funcstat	try_to_take_forks(t_personal *own_p)
{
	...
	{
		set_errstat_simustat_and_print_errmsg(own_p->d, MUTEX_LOCK_ERROR, ERRMSG_MUTEX_LOCK);
		return (1);
	}
	__DEBUG_PRINT_THREAD_INFO__(own_p->d, own_p);
...
static t_funcstat	philo_eat(t_personal *own_p)
{
	...
	{
		set_errstat_simustat_and_print_errmsg(own_p->d, MUTEX_UNLOCK_ERROR, ERRMSG_MUTEX_UNLOCK);
		return (1);
	}
	__DEBUG_PRINT_THREAD_INFO__(own_p->d, own_p);
